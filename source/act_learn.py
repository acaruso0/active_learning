...
if settings.restart:
    restart = pd.read_csv(settings.restart_file, sep='\t')
    idx_now = restart['idx'].to_numpy()
    energies = restart['energy'].to_numpy()
    error = restart['error'].to_numpy()
    Y_train[idx_now] = energies
    err_train[idx_now] = error
    idx_left = idx_left[~np.in1d(idx_left,
                                           idx_now)]
...
while settings.t < settings.tmax:
    ...
    calc_energy = Energy(coords, idx_pick, settings.t)
